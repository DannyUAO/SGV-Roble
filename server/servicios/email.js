...
/**
 * Envía correo al residente cuando un visitante sale del edificio.
 */
async function notificarSalida({ correoResidente, nombreResidente, apartamento, visitante, documento, vigilante, horaIngreso, horaSalida }) {
  const formatHora = (h) => new Date(h).toLocaleString('es-CO', {
    timeZone: 'America/Bogota',
    dateStyle: 'short',
    timeStyle: 'short',
  });

  const duracionMin = Math.round((new Date(horaSalida) - new Date(horaIngreso)) / 60000);
  ...
  await transporter.sendMail({
    from: `"SGV Roble" <${process.env.GMAIL_USER}>`,
    to: correoResidente,
    subject: `Su visitante ha salido del edificio · Apto ${apartamento}`,
    html: `
      <div style="font-family:Arial,sans-serif;max-width:520px;margin:auto;border:1px solid #e0e0e0;border-radius:8px;overflow:hidden;">
        <div style="background:#1a1a2e;padding:20px 24px;">
          <h2 style="color:#fff;margin:0;">🏢 SGV Roble</h2>
        ...
        <div style="padding:24px;">
          <p style="font-size:15px;">Estimado/a <strong>${nombreResidente}</strong>,</p>
          <p style="font-size:15px;">Le informamos que el siguiente visitante ha salido del edificio:</p>
          <table style="width:100%;border-collapse:collapse;margin:16px 0;">
            <tr style="background:#f5f5f5;">
              <td style="padding:10px 14px;font-weight:600;">Visitante</td>
              <td style="padding:10px 14px;">${visitante}</td>
            ...
            <tr style="background:#f5f5f5;">
              <td style="padding:10px 14px;font-weight:600;">Apartamento visitado</td>
              <td style="padding:10px 14px;">${apartamento}</td>
            </tr>
            <tr>
              <td style="padding:10px 14px;font-weight:600;">Hora de ingreso</td>
              <td style="padding:10px 14px;">${formatHora(horaIngreso)}</td>
            </tr>
            <tr style="background:#f5f5f5;">
              <td style="padding:10px 14px;font-weight:600;">Hora de salida</td>
              <td style="padding:10px 14px;">${formatHora(horaSalida)}</td>
            </tr>
            <tr>
              <td style="padding:10px 14px;font-weight:600;">Duración de la visita</td>
              <td style="padding:10px 14px;">${duracion}</td>
            </tr>
            <tr style="background:#f5f5f5;">
              <td style="padding:10px 14px;font-weight:600;">Registrado por</td>
              <td style="padding:10px 14px;">${vigilante}</td>
...
module.exports = { notificarIngreso, notificarSalida };
